...
$sql_kategori = "SELECT id, kategori_adi FROM kategoriler";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $kategori_id = $conn->real_escape_string($_POST['kategori']);
    ...
    $target_dir = "../img/yuklemeler/";
    $target_file = $target_dir . strtolower(str_replace(' ', '_', $oyun_adi)) . '.png';
    ...
    // Dosyanın zaten mevcut olup olmadığını kontrol etme
    ...
    if ($_FILES["gorsel"]["size"] > 10000000) {
        echo "Üzgünüz, dosyanız çok büyük.";
        $uploadOk = 0;
    }

    // Belirli dosya türlerine izin verme
    ...
    // uploadOk değeri 0 ise dosya yüklenmedi
    if ($uploadOk == 0) {
        echo "Üzgünüz, dosyanız yüklenmedi.";
    // Tüm kontrolleri geçerse dosyayı yükle
    } else {
        if (move_uploaded_file($_FILES["gorsel"]["tmp_name"], $target_file)) {
            $gorsel = $target_file;
            $sql = "INSERT INTO oyunlar (oyun_adi, gorsel, aktif_oyuncu_sayisi, kategori_id, aciklama, yapimci, yayinlanma_tarihi, rating)
                    VALUES ('$oyun_adi', '$gorsel', '$aktif_oyuncu_sayisi', '$kategori_id', '$aciklama', '$yapimci', '$yayinlanma_tarihi', '$rating')";

            if ($conn->query($sql) === TRUE) {
                echo "Yeni kayıt başarıyla oluşturuldu";

                // Dinamik sayfa oluşturma
                $oyun_id = $conn->insert_id;
                $oyun_adi_dosya = strtolower(str_replace(' ', '_', $oyun_adi));
                $oyun_dosyasi_yolu = "../oyunlar/" . $oyun_adi_dosya . ".php";
...
<body>
    ...
    <script src="../js/main.js"></script>
                ...
                if (file_put_contents($oyun_dosyasi_yolu, $oyun_dosyasi_icerik)) {
                    echo "Oyun sayfası başarıyla oluşturuldu: " . $oyun_dosyasi_yolu;
                } else {
                    echo "Oyun sayfası oluşturulurken hata meydana geldi.";
                }
            } else {
...
<body>
    ...
    <div class="container">
        <div class="row">
                <div class="col-lg-12">
                    <div class="oyun_form" style="margin-bottom: 50px;">
                        <form action="" method="post" enctype="multipart/form-data">
                            <div class="oyun_item">
                                <label for="oyun_adi"></label>
                                <input type="text" id="oyun_adi" name="oyun_adi" required placeholder="Oyun Adı">
                            </div>
                            <div class="oyun_item">
                                <label for="gorsel"></label>
                                <input type="file" id="gorsel" name="gorsel" accept="image/*" required style="background: none;">
                            </div>
                            <div class="oyun_item">
                                <label for="aktif_oyuncu_sayisi"></label>
                                <input type="number" id="aktif_oyuncu_sayisi" name="aktif_oyuncu_sayisi" required placeholder="Aktif Oyuncu Sayısı">
                            </div>
                            <div class="oyun_item">
                            ...
                            <div class="oyun_item">
                                ...
                                <textarea id="aciklama" name="aciklama" rows="4" required placeholder="Açıklama"></textarea>
                            </div>
                            <div class="oyun_item">
                                <label for="yapimci"></label>
                                <input type="text" id="yapimci" name="yapimci" required placeholder="Yapımcı">
                            </div>
                            <div class="oyun_item">
                                <label for="yayinlanma_tarihi"></label>
                                <input type="date" id="yayinlanma_tarihi" name="yayinlanma_tarihi" required>
                            </div>
                            <div class="oyun_item">
                                <label for="rating"></label>
                                <input type="number" step="0.1" id="rating" name="rating" required placeholder="Rating (10 üzerinden)">
                            </div>
